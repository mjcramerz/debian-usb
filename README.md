...
Then the app collects local paths or deferred downloads, source role, optional modules/overlays/preseed, Live tool groups, persistence/encryption, boot settings, and the target whole disk. Local sources are inspected read-only; remote source inspection remains pending until execution. A colored technical table shows the device path/model/capacity/serial and every selection, and saves a declarative plan. Only final confirmation starts preparation. Missing crypt/LUKS support is incorporated automatically when required, not through an early rebuild prompt. Single-OS, Multi-OS, saved-plan replay/edit, and update use the common preparation pipeline. Raw/as-is writes bypass remastering.

Interactive menus support arrow navigation and highlighted selections; numeric input and plain non-TTY/NO_COLOR fallback remain available. See the linked refactor document for the precise preparation and failure boundaries.
...
Managed downloads remain separated by URL key under `/data/downloads/debian-usb/iso/` or `/data/downloads/debian-usb/boot/`. Reviewed plans freeze the configured key and URL, reject changed configuration, and do not reuse a cached payload associated with another URL. Manifest-resolved release ISOs and pinned or dated URLs may reuse a matching cached payload. Unresolved moving aliases such as `current`, `daily`, `arch-latest`, and `lastsuccessful` are refreshed during reviewed execution so a newly resolved Netinst ISO cannot be paired with stale hd-media kernel/initrd assets. The legacy direct helper API without `--expected-url` retains its offline cache-first compatibility behavior.
...
The shared ISO-store root is also scrubbed and validated so `/.disk`, `/live`, `/casper`, `/install`, `/install.amd`, `/d-i`, and `/debian-installer` cannot masquerade as metadata for any one payload. Those paths may exist only inside their opaque profile-specific ISO. Recovery or administration packages can be selected during USB planning and are incorporated in the consolidated post-confirmation Live remaster, or included in a custom source ISO.
...
1. collects the build selection without installing host dependencies; missing dependencies are installed only after the final build confirmation
...
The Build Custom ISO prompt flow also now supports pre-build kernel evidence checks for the exact module set you intend to use. You can point it at a specific kernel version, inspect current `CONFIG_*` symbols plus module-file presence from `/lib/modules/<version>/...`, and optionally let it download and extract matching kernel packages into the managed cache when that version is not already present on the host. Managed installer preparation derives this version from the selected installer initrd, not from the running host kernel; the existing privileged helper boundary downloads `linux-image-<installer-ABI>` (including an explicit APT package version when required) and extracts it into the cache rather than substituting modules from a different host ABI.
...
At Debian Live boot, the APT policy runs from inside the rootfs, so it remains available when module-only RAM boot hides medium hooks. It filters stale local sources, preserves enabled network sources, and validates the real ISO's Release metadata, package indexes, and referenced `.deb` files before exposing a local Deb822 source. It can reopen only the exact managed `findiso` payload from the exact UUID-addressed USB store, read-only; it never scans sibling Multi-OS ISOs or copies the package pool into RAM. An incomplete or missing local repository is disabled instead of leaving APT probing nonexistent `Packages.xz` paths. The rootfs mount is not used as a substitute package repository.
...
The Debian Build Custom ISO host dependencies are **not** installed during `make install`. They are installed lazily after the final confirmation in `Build Custom ISO` or the standalone rebuild workflow.
