...
Managed downloads remain separated by URL key under `/data/downloads/debian-usb/iso/` or `/data/downloads/debian-usb/boot/`. Reviewed plans freeze the configured key and URL, reject changed configuration, and do not reuse a cached payload associated with another URL. Moving aliases are resolved at execution; use authenticated local artifacts or pinned URLs for reproducibility. The legacy direct helper API without `--expected-url` retains its cache-first compatibility behavior.
